...
from workbench.tools.formats import local_date_format, pretty_due, markdownify
...
class Project(Model):
    ...
    def overview(self):
        ...
        return {
            "logged": LoggedHours.objects.filter(task__project=self)
            .order_by()
...
class Task(Model):
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.PROTECT, verbose_name=_("created by"), related_name="+"
    )
    ...
    type = models.CharField(
        _("type"), max_length=20, choices=TYPE_CHOICES, default=TASK
    )
    priority = models.PositiveIntegerField(
        _("priority"), choices=PRIORITY_CHOICES, default=NORMAL
    )
    owned_by = models.ForeignKey(
        User,
        on_delete=models.PROTECT,
    ...
    status = models.PositiveIntegerField(
        _("status"), choices=STATUS_CHOICES, default=INBOX
    )
    ...
    def overview(self):
        ...
        hours_per_task = {
            ...
            for row in LoggedHours.objects.filter(task__service=self.service)
            .order_by()
            .values("task")
            .annotate(Sum("hours"))
        }
